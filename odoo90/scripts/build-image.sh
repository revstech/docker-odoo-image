...
. /usr/share/revs-docker-internal/odoo90/library.sh
. /etc/lsb-release
...
ODOO_DEPENDENCIES="git+https://github.com/revstech/odoo@9.0 \
                   git+https://github.com/revstech/server-tools@9.0"
DEPENDENCIES_FILE="/usr/share/revs-docker-internal/odoo90/9.0-full_requirements.txt"
DPKG_DEPENDS="nodejs \
              phantomjs \
              antiword \
              python-dev \
...
PIP_DEPENDS_EXTRA="requirements-parser==0.1.0 \
                   mercurial==3.2.2 \
                   setuptools==33.1.1 \
                   git+https://github.com/revstech/pylint-odoo@master#egg=pylint-odoo"
...
echo "include = '/etc/postgresql-common/common-revstech.conf'" >> /etc/postgresql/${PSQL_VERSION}/main/postgresql.conf
